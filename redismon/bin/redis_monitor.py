...
class RedisFalconMonitor(object):
  ...
  def __init__(self, addr, port, password, cluster_name):
    ...
    logging.config.fileConfig("/home/mi/repo/redismon/conf/logging.ini")
  ...
  def ping_redis(self):
    ...
    if redis_is_alive == 0:  # If redis is dead, update the alive metrice here.
      redis_alive_data = [{"endpoint": self.addr, "metric": "redis_alive", "tags": self.tags, "timestamp": upload_ts,
                           "value": redis_is_alive, "step": 60, "counterType": "GAUGE"}]
...
def main():
  ...
  f = open("/home/mi/repo/redismon/conf/redismon.conf")
  y = yaml.load(f)
  f.close()
  redis_items = y["items"]
